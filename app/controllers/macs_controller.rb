class MacsController < ApplicationController
  ...
  def index
    ...
    if @search_term.present?
      @macs = @macs.includes(:user)
              .where("macs.comment LIKE ? OR macs.mac LIKE ? or users.email LIKE ?",
                     "%#{@search_term}%", "%#{@search_term}%", "%#{@search_term}%")
              .references(:user)
    end

    # paginate and sort
    @macs = @macs.includes(:user).sorted(params[:sort])
            .paginate(page: params[:page], per_page: 20).references(:user)
  end
